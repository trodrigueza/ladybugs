...
from apps.seguridad.decoradores import admin_requerido, login_requerido
from apps.seguridad.models import Rol, Usuario
from apps.seguridad.servicios.autenticacion import (
    autenticar_usuario,
    registrar_logout,
)
from apps.seguridad.servicios.FormularioSocio_Membresia import (
    SocioForm,
    SocioMembresiaForm,
    UsuarioForm,
)
...
def gestionar_usuarios_view(request):
    ...
    if tipo_selected == "todos" or tipo_selected == "Socio":
        ...
        if plan_id and plan_id != "todos":
            socios = (
                Socio.objects.prefetch_related(
                    Prefetch(
                        "membresias", queryset=memb_qs, to_attr="membresias_ordered"
                    )
                )
                .filter(membresias__PlanID_id=plan_id)
                .distinct()
            )
        else:
...
def crear_usuario_view(request, tipo_rol):
    ...
    return render(
        request,
        "Administrador/crearUsuario.html",
        {"form": usuario_form, "tipo_rol": tipo_rol},
    )
...
def crear_socio_view(request):
    if request.method == "POST":
        ...
        if socio_form.is_valid():
            ...
            if not password:
                messages.error(
                    request, "La contraseña es obligatoria al crear un socio."
                )
                return render(
                    request, "Administrador/crearSocio.html", {"form": socio_form}
                )
            ...
            messages.success(
                request,
                f"Socio {socio.NombreCompleto} y usuario creados correctamente.",
            )
        ...
        if socio_form.is_valid():
            ...
            if password:
                try:
                    ...
                    messages.success(
                        request,
                        f"Socio {socio.NombreCompleto} y contraseña actualizados correctamente.",
                    )
                except Usuario.DoesNotExist:
                    messages.warning(
                        request,
                        f"Socio actualizado, pero no se encontró usuario asociado para actualizar contraseña.",
                    )
            else:
                messages.success(
                    request, f"Socio {socio.NombreCompleto} actualizado correctamente."
                )
...
def editar_usuario_view(request, usuario_id):
    ...
    if request.method == "POST":
        ...
        if usuario_form.is_valid():
            usuario_form.save()
            messages.success(
                request, f"Usuario {usuario.Email} actualizado correctamente."
            )
            return redirect("gestionar_usuarios")
    else:
    ...
    return render(
        request,
        "Administrador/editarUsuario.html",
        {"form": usuario_form, "usuario": usuario},
    )
...
def eliminar_entidad_view(request, tipo, entidad_id):
    ...
    try:
        if tipo_normalizado == "socio":
            ...
            return JsonResponse(
                {"success": False, "error": "Tipo de entidad no válido"}, status=400
            )
        ...
        return JsonResponse(
            {"success": False, "error": "Socio no encontrado"}, status=404
        )
    except Usuario.DoesNotExist:
        return JsonResponse(
            {"success": False, "error": "Usuario no encontrado"}, status=404
        )
